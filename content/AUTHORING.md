...
2. The **filename becomes the URL**: `content/blog/best-iptv-players.mdx` → `/blog/best-iptv-players`.
...
author: "EasyIPTV Team"       # optional — defaults to the site's editorial byline
category: "Tutorials"         # optional — shows on the card (e.g. Tutorials, Guides, News, Streaming Tips)
...
related:                      # optional — internal links (SEO) + external sources
  - label: "IPTV Installation Guide"
    href: "/installation-guide"                    # internal → "Related Reading"
  - label: "TechRadar streaming report"
    href: "https://www.techradar.com/…"            # external (http) → "Sources"
...
Body text. **Bold**, *italic*, and [links](/installation-guide) all work.
...
**Photo (simple):**
```mdx
![Descriptive alt text for SEO and accessibility](https://…/screenshot.jpg)
```

**Photo with caption:**
```html
<figure>
  <img src="https://…/screenshot.jpg" alt="Descriptive alt text" />
  <figcaption>A short caption.</figcaption>
</figure>
```

**YouTube video (great for setup tutorials):**
```html
...
**Any other embed** (maps, players, tweets) — paste its `<iframe>` and it renders.
...
**Length**
- **Blog / tutorial / guide:** aim for **~1,500–2,500 words** (target ~2,000). Depth wins.
...
**Media**
- **At least one image** per post (a cover via `image:` and/or an in-body screenshot).
- For setup/how-to posts, embed a **video** or step screenshots where they genuinely help.
...
- **2–4 internal links** to relevant pages (e.g. `/installation-guide`, `/faq`, the
  homepage) via body links and/or `related`. This is the biggest ranking lever.
...
- **Original writing only.** Never paste content from another site — it's duplicate
  content and gets penalized.
- Write from experience: specific devices, apps, steps, settings, honest caveats. No fluff.
- Stay **on-topic** for the site's niche (IPTV, streaming, cord-cutting, device setup).
